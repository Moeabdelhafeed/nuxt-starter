          ...
          <div v-if="exists && availableChannels.length > 1" class="grid gap-2">
            ...
            <div class="flex gap-2">
              <Button
                ...
                type="button"
                size="sm"
                :variant="form.type === ch ? 'default' : 'outline'"
                @click="form.type = ch"
              >
                {{ t(`channel_${ch}`, ch) }}
              </Button>
...
const { identifierLabel, identifierInputType, identifierPlaceholder } = useAuthConfig()
const { t } = useLang()
...
const client = useApi()
const form = ref({ identifier: '', type: '' })
  ...
  debounce = setTimeout(async () => {
    try {
      ...
      if (availableChannels.value.length === 1) {
        form.value.type = availableChannels.value[0]
      }
    } catch {
      exists.value = null
      availableChannels.value = []
...
const onSubmit = async () => {
  ...
  try {
    ...
    if (form.value.type) body.type = form.value.type
    ...
    navigateTo({
      ...
      query: { identifier: form.value.identifier }
    })
  } catch (error) {
